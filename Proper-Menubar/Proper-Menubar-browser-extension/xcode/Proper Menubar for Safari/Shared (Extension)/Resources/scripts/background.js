	...
	if(request.name == "stefanproper"){
		...
		chrome.tabs.query({active: true, currentWindow: true}, function(tabs){
			...
			if(exbrowser != "safari"){
				...
				// Safari: close current tab, then open a new one
				chrome.tabs.remove(tab.id);
				chrome.tabs.create({});
			}
		});
		...
		if(exbrowser == "firefox"){
			...
			chrome.tabs.query({active: true, currentWindow: true}, function(tabs){
				...
				chrome.scripting.executeScript({
					target: {tabId: tab.id},
					func: function(){
						// Replace this with your real getHtmlContent logic
		...
		// Safari web browser zoom at 10% steps
		chrome.tabs.query({active: true, currentWindow: true}, function(tabs){
			var tabId = tabs[0].id;
			chrome.tabs.getZoom(tabId, function(zoomFactor){
				// Increase zoom by 0.10
				var newZoom = zoomFactor + 0.10;
		...
		chrome.tabs.query({active: true, currentWindow: true}, function(tabs){
			var tabId = tabs[0].id;
			chrome.tabs.getZoom(tabId, function(zoomFactor){
				// Decrease zoom by 0.10, ensure it does not go below 0.1
				var newZoom = Math.max(0.1, zoomFactor - 0.10);
				chrome.tabs.setZoom(tabId, newZoom);
			});
		});
		...
		chrome.tabs.query({active: true, currentWindow: true}, function(tabs){
			var tab = tabs[0];
			if(exbrowser == "safari"){
				chrome.tabs.sendMessage(tab.id, {action: "goviewsource"});
			}else{
				chrome.tabs.create({url: "view-source:" + tab.url});
			}
		});
	}else if(request.name == "stefanbrowserversion"){
		chrome.tabs.query({active: true, currentWindow: true}, function(tabs){
			var tab = tabs[0];
			chrome.tabs.sendMessage(tab.id, {action: "gobrowserversion"});
		});
...
chrome.storage.sync.get(["icon"], function(items){
	if(items["icon"] == undefined){
		if(exbrowser == "safari"){
			items["icon"] = "/images/icon38safari.png";
		}else{
