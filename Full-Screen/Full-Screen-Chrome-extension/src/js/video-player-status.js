...
(fsCinema = {
	...
	initialize: function(){
		...
		if(MutationObserver){
			...
			document.addEventListener("DOMNodeRemoved", fsCinema.initvideoinject, false);
			document.addEventListener("DOMNodeInserted", fsCinema.initvideoinject, false);
		}
	},
	initvideoinject: function(){
		...
		if(youtubeplayer !== null){ // YouTube video element
			var interval = window.setInterval(function(){ if(youtubeplayer.pause || youtubeplayer.pauseVideo){ window.clearInterval(interval); if(youtubeplayer.pauseVideo){ youtubeplayer.addEventListener("onStateChange", "fsCinema.playerStateChange"); } } }, 10);
		}
