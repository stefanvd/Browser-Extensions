...
chrome.browserAction.onClicked.addListener(function(tab) {var action_url = "javascript:window.print();";chrome.tabs.update(tab.id, {url: action_url});});

var icon;
chrome.storage.sync.get(['icon'], function(items){
  if(icon == undefined){icon = "icons/default@2x.png";}
  chrome.browserAction.setIcon({"path":items["icon"]});
});// chrome storage end

chrome.storage.onChanged.addListener(function(changes, namespace) {
   for (key in changes) {
          var storageChange = changes[key];
          if(changes['icon']){if(changes['icon'].newValue){chrome.browserAction.setIcon({"path":changes['icon'].newValue});}}
    }
})
