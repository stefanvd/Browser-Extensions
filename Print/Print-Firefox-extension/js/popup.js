...
$("btnsavepage").addEventListener('click', function() {
    chrome.tabs.query({active: true, currentWindow: true}, function(arrayOfTabs) {
        ...
        var activeTabId = activeTab.id;
            chrome.tabs.executeScript(activeTabId, {file: "js/saveas.js"})
    });
});
$("btnprintpage").addEventListener('click', function() {
    chrome.tabs.query({active: true, currentWindow: true}, function(arrayOfTabs) {
            var activeTab = arrayOfTabs[0];
            var activeTabId = activeTab.id;
            chrome.tabs.executeScript(activeTabId, {file: "js/print.js"})
    });
});
$("btnprintpageblack").addEventListener('click', function() {
    chrome.tabs.query({active: true, currentWindow: true}, function(arrayOfTabs) {
            var activeTab = arrayOfTabs[0];
            var activeTabId = activeTab.id;
            chrome.tabs.executeScript(activeTabId, {file: "js/printblack.js"})
    });
});
$("btnrate").addEventListener('click', function() {chrome.tabs.create({url: writereview, active:true})});
});
