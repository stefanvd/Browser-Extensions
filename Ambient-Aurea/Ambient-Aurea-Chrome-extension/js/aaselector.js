...
count = response['count'];if(!sharebar)sharebar = 0;
slideshow = response['slideshow'];
slideshowrefresh = response['slideshowrefresh'];if(!slideshowrefresh)slideshowrefresh = 5;
...
var j = 0;
var blackon = $("stefanvdaagallery");
if(blackon){
	if(fadeout == 'true'){fader('hide');}
	else{var dodo = $("stefanvdaalightareoff1");document.body.removeChild(dodo);}
	cleanup();
}else{
// build gallery frame
...
var height = img[i].naturalHeight;
var width = img[i].naturalWidth;
if((width > 1 || height > 1) && (img[i].src != "")){
...
newaaimage.onload = function(e) {resizeframe();refreshambilight();refreshcounter();};
newaaimage.setAttribute('id','stefanvdaagalleryimage' + j + '');
newaaimage.setAttribute('class','stefanvdaagalleryimagehide');
newaaimage.src = img[i].src;
newaagallery.appendChild(newaaimage);
}
}

// first receive the show
$('stefanvdaagalleryimage' + '1' + '').setAttribute('class','stefanvdaagalleryimageshow');
sourceImageWidth = $('stefanvdaagalleryimage' + '1' + '').clientWidth;
sourceImageHeight = $('stefanvdaagalleryimage' + '1' + '').clientHeight;
...
$('stefanvdaagalleryimage' + j + '').setAttribute('class','stefanvdaagalleryimagehide');
if(j == 1){j = kmax;}
else {j-=1;}
$('stefanvdaagalleryimage' + j + '').setAttribute('class','stefanvdaagalleryimageshow');
resizeframe();refreshambilight();refreshcounter();
});
document.body.appendChild(newaagalleryleft);
// button right
...
document.body.appendChild(newaagalleryright);

window.addEventListener('keydown', function(e) {
	var lightareoff = $('stefanvdaalightareoff1');
	if (lightareoff != null) {
		if (e.which == 37) {
$('stefanvdaagalleryimage' + j + '').setAttribute('class','stefanvdaagalleryimagehide');
if(j == 1){j = kmax;}
...
// reset j to 1 first slide in the counter text
j = 1;

if(count == 'true'){
var newcounter = document.createElement('div');
newcounter.setAttribute('id','stefanvdaagallerycounter');
newcounter.innerText = "" + j + "/" + kmax + ""
document.body.appendChild(newcounter);
}

function refreshcounter(){
if($('stefanvdaagallerycounter')){$('stefanvdaagallerycounter').innerText = "" + j + "/" + kmax + "";}
}

function resizeframe(){
var newscreenheight = document.documentElement.clientHeight;
var newscreenwidth = document.documentElement.clientWidth;
if($('stefanvdaagalleryimage' + j + '')){
sourceImageWidth = $('stefanvdaagalleryimage' + j + '').width;
sourceImageHeight = $('stefanvdaagalleryimage' + j + '').height;
newaagallery.style.left = (newscreenwidth/2) - (sourceImageWidth/2) + "px";
newaagallery.style.top = (newscreenheight/2) - (sourceImageHeight/2) + "px";
}}
...
function refreshambilight(){
	var totlshowtime = document.querySelectorAll('img.stefanvdaagalleryimageshow');
...
document.body.appendChild(newdivslideshow);
...
$('stefanvdaaplaybutton').style.display = "none";$('stefanvdaapreviousbutton').style.display = "";slideshowtimernext();
});
...
$('stefanvdaaplaybutton').style.display = "";$('stefanvdaapreviousbutton').style.display = "none";window.clearTimeout(countss);
});
...
alert("Sorry no image found on this web page! Try another web page.");
}
