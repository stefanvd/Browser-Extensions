...
function save_options(){
    ...
    safari.self.tab.dispatchMessage("update_setting", { "allzoom": $('allzoom').checked, "optionskipremember": $('optionskipremember').checked, "contextmenus": $('contextmenus').checked, "badge": $('badge').checked, "defaultzoom": $('defaultzoom').value, "steps": $('steps').value, "lightcolor": $('lightcolor').value, "zoomchrome": $('zoomchrome').checked, "zoomweb": $('zoomweb').checked, "websitezoom": JSON.stringify(websitezoom)});
}
...
safari.self.addEventListener("message", function(e) {
        ...
        if(safari.extension.settings['defaultzoom']){$('defaultzoom').value = safari.extension.settings['defaultzoom'];$('slider').value = safari.extension.settings['defaultzoom'];}	
        else $('defaultzoom').value = 100;
        if(safari.extension.settings['steps']){$('steps').value = safari.extension.settings['steps'];}	
        else $('steps').value = 10;
        ...
        if(safari.extension.settings['allzoom'] == true)$('allzoom').checked = true;
        if(safari.extension.settings['optionskipremember'] == true)$('optionskipremember').checked = true;
        if(safari.extension.settings['contextmenus'] == true)$('contextmenus').checked = true;
        if(safari.extension.settings['badge'] == true)$('badge').checked = true;
        if(safari.extension.settings['zoomchrome'] == true){$('zoomchrome').checked = true;$('zoomweb').checked = false;}
        if(safari.extension.settings['zoomweb'] == true){$('zoomweb').checked = true;$('zoomchrome').checked = false;}

        // if empty use this
        var websitezoom = safari.extension.settings['websitezoom'];
...
window.localStorage['countremember'] = countremember;
	...
	$("version_number").innerText = extensionversion;
...
// Reset settings
$("resetzoom").addEventListener('click', function() {window.localStorage.clear();location.reload()});
