...
if (allzoom == true) {
    chrome.windows.getCurrent(function (w) {
        ...
        function (response) {
            ...
            if(zoomchrome == true){
                chrome.tabs.setZoom(tabId, allzoomvalue);
            }else{
                chrome.tabs.executeScript(tabId,{code:"document.body.style.zoom=" + allzoomvalue});
            }
            if(badge == true){
      ...
      var editzoom = websitezoom[atbbuf[i]]/100;
          chrome.windows.getCurrent(function (w) {
              ...
              function (response) {
                  tabId = response.id;
                  if (zoomchrome == true) {
                      chrome.tabs.setZoom(tabId, editzoom);
                  } else {
                      chrome.tabs.executeScript(tabId, { code: "document.body.style.zoom=" + editzoom });
                  }
                  if (badge == true) {
                      chrome.browserAction.setBadgeBackgroundColor({ color: lightcolor });
    ...
    chrome.windows.getCurrent(function (w) {
        ...
        function (response) {
            ...
            if (zoomchrome == true) {
                ...
                chrome.tabs.sendMessage(tabId, { text: 'getwebzoom' }, function (info) {
                    if (info == null) { info = 1 }
...
if (respage == "zoompage") {
    ...
    chrome.tabs.query({ active: true }, function (tabs) {
        tabId = tabs[0].id;
        if(zoomchrome == true){
            chrome.tabs.setZoom(tabId, czl/100);
        }else{
            chrome.tabs.executeScript(tabId,{code:"document.body.style.zoom=" + czl/100});
        }
        if(badge == true){
            chrome.browserAction.setBadgeBackgroundColor({color:lightcolor}); 
