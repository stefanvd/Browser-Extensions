...
document.addEventListener("DOMContentLoaded", function(){
	...
	$("snowAmount").addEventListener("input", function(){
		...
		var val = parseInt($("snowAmount").value);
		chrome.storage.sync.set({snowAmount: val});
		chrome.runtime.sendMessage({name: "livesnowAmount", value: val});
	}, false);

	$("snowColor").addEventListener("input", function(){
		var val = $("snowColor").value;
		chrome.storage.sync.set({snowColor: val});
		chrome.runtime.sendMessage({name: "livesnowColor", value: val});
	}, false);
	...
	$("colorOption").addEventListener("change", function(){
		...
		var val = $("colorOption").value;
		chrome.storage.sync.set({colorOption: val});
		chrome.runtime.sendMessage({name: "livecolorOption", value: val});
	}, false);
	...
	$("snowSize").addEventListener("input", function(){
		var val = parseInt($("snowSize").value);
		chrome.storage.sync.set({snowSize: val});
		chrome.runtime.sendMessage({name: "livesnowSize", value: val});
	}, false);

	$("snowSpeed").addEventListener("input", function(){
		var val = parseInt($("snowSpeed").value);
		chrome.storage.sync.set({snowSpeed: val});
		chrome.runtime.sendMessage({name: "livesnowSpeed", value: val});
	}, false);

	$("windDirectionControl").addEventListener("change", function(){
		var val = $("windDirectionControl").checked;
		chrome.storage.sync.set({windDirectionControl: val});
		chrome.runtime.sendMessage({name: "livewindDirectionControl", value: val});
	}, false);

	$("snowOnBottom").addEventListener("change", function(){
		var val = $("snowOnBottom").checked;
		chrome.storage.sync.set({snowOnBottom: val});
		chrome.runtime.sendMessage({name: "livesnowOnBottom", value: val});
	}, false);
