...
chrome.storage.sync.get(['marqueebehaviour','direction','scrollamount','japan','xminutes','favoritestock','favo1','favo2','favo3','favo4','getinfovaluestock','getinfovaluepercent','getinfovaluemc','getfontfamily','getfontsize','excludedstock','countremember','optionskipremember','lightcolor','redcolor','greencolor','textcolor'], function(items){
		marqueebehaviour = items['marqueebehaviour']; if (!marqueebehaviour) marqueebehaviour = 'scroll';
		direction = items['direction']; if (!direction) direction = 'left';
		scrollamount = items['scrollamount']; if (!scrollamount) scrollamount = '35';
		japan = items['japan'];
		xminutes = items['xminutes']; if (!xminutes) xminutes = '15';
		favoritestock = items['favoritestock'];
		favo1 = items['favo1']; if (!favo1) favo1 = 'EURUSD=X';
		favo2 = items['favo2']; if (!favo2) favo2 = 'USDJPY=X';
		favo3 = items['favo3']; if (!favo3) favo3 = 'GBPUSD=X';
		favo4 = items['favo4']; if (!favo4) favo4 = 'USDCAD=X';
		getinfovaluestock = items['getinfovaluestock']; if (!getinfovaluestock) getinfovaluestock = false;
		getinfovaluepercent = items['getinfovaluepercent']; if (!getinfovaluepercent) getinfovaluepercent = true;
		getinfovaluemc = items['getinfovaluemc']; if (!getinfovaluemc) getinfovaluemc = false;
		getfontfamily = items['getfontfamily']; if (!getfontfamily) getfontfamily = 'jd_lcd_roundedregular';
		getfontsize = items['getfontsize']; if (!getfontsize) getfontsize = '27';
		lightcolor = items['lightcolor']; if (!lightcolor) lightcolor = '#000000';
		redcolor = items['redcolor']; if (!redcolor) redcolor = '#f00000';
		greencolor = items['greencolor']; if (!greencolor) greencolor = '#008000';
		textcolor = items['textcolor']; if (!textcolor) textcolor = '#ffffff';
		lightcolor = items['lightcolor']; if (!lightcolor) lightcolor = '#000000';
		excludedstock = items['excludedstock']; if (!excludedstock) excludedstock = JSON.stringify({'GOOG': true,'MSFT': true,'YHOO': true,'AAPL': true,'^DJX': true,'^SPX': true,'Gold': true,'Oil': true,'QQQX': true,'EURUSD=X': true,'^BFX': true});
